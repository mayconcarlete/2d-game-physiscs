...
void Graphics::DrawPolygon(int x, int y, const std::vector<Vec2>& vertices, Uint32 color) {
    for (int i = 0; i < vertices.size(); i++) {
        ...
        float xCurrIndex = vertices[currIndex].GetX();
        float yCurrIndex = vertices[currIndex].GetY();
        float xNextIndex = vertices[nextIndex].GetX();
        float yNextIndex = vertices[nextIndex].GetY();
        lineColor(renderer, xCurrIndex, yCurrIndex, xNextIndex, yNextIndex, color);
    }
